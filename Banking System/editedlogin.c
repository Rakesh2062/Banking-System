...
int editedlogin()
{
    ...
    while (file = readdir(folder))    // it simply opens the files one by one till there exists file in the directory
    {
        ...
        if (strstr(file->d_name, ".txt") != NULL)    // file->d_name means the name of file and strstr() checks the first d_name has either .txt in its name or not and if there is .txt then it opens or it will give NULL and go out of if statement. Also we are only taking .txt files because the date (acc_no, password, username, amount) are stored in .txt files and .c and .exe files are useless for us.
        {
            ...
            fp = fopen(file->d_name, "rb");   // opening file which has name as file->d_name with read mode.
            if (fp == NULL)  // file doesnot open then we will return 0.
            {
                return 0;
            }
            fread(&d1, sizeof(d1), 1, fp);  // it reads the data from the file and stores it in d1 structure.
           //no need for this but not removing it, fscanf(fp, "%s", l[0].acc_no); // it stores the first line in the files (for us the first line is account number) into l[0].acc_no
           // same here, fscanf(fp, "%s", l[0].pass); // when we use again fscanf() then, as it has already read the first line now it will read second line (if we again use fscanf() below this then it will read third line) which has password stored in it and stores it in l[0].pass
            ...
            {
                return i+1 ;
                // all same except this, return i + 1; 
             ...
             // we will inc. the value of i by 1 and now the value of i will 1.
        i++;
        }
    }

    closedir(folder); // as we have opened the directory we are closing it also.
    return 2; // if neither of the above code runs then it will return 0. 
